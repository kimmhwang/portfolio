...
const SITE_CONFIG = {
  ...
  colorThemes: {
    ...
    accessibleLight: {
      ...
      primary: "#b45309", secondary: "#1d4ed8", accent: "#c2410c",
      bg: "#ffffff", surface: "#f5f5f5", surfaceHover: "#e8e8e8",
      border: "#a1a1aa", text: "#000000", textMuted: "#3f3f46", textDim: "#52525b",
    },
  },
  ...
  return (
    <div>
      ...
      <section style={{ padding: "40px 0", borderTop: `1px solid ${C.border}` }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: m ? "1fr 1fr" : "repeat(auto-fill, minmax(220px, 1fr))", gap: 14 }}>{filtered.length > 0 ? filtered.map((item, i) => <GalleryCard key={item.id || i} item={item} onAction={handleGalleryClick} />) : <div style={{ gridColumn: "1 / -1", padding: 36, textAlign: "center", color: C.textDim, fontSize: 13 }}>No items yet.</div>}</div>
      </section>
...
const AboutPage = ({ initialSection }) => {
  ...
  return (
    <div style={{ paddingTop: 28, paddingBottom: 40 }}>
      ...
      {/* MENTORS */}
      <section ref={registerRef("mentors")} style={{ paddingTop: 32, borderTop: `1px solid ${C.border}` }}><div style={{ background: `linear-gradient(135deg, ${C.accent}08, ${C.accent}04)`, borderRadius: 16, border: `1px solid ${C.accent}20`, padding: m ? "20px 16px" : "28px 24px" }}><div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 6 }}><span style={{ fontSize: 20 }}>👥</span><h2 style={{ fontSize: 20, fontWeight: 700, margin: 0, color: C.text }}>Mentors</h2></div><p style={{ fontSize: 13, color: C.textMuted, margin: "0 0 20px" }}>People who shaped my journey</p>{MENTORS.length > 0 ? <div style={{ display: "grid", gridTemplateColumns: m ? "1fr" : "1fr 1fr", gap: 12 }}>{MENTORS.map((mt, i) => { const W = mt.url ? "a" : "div"; return <W key={i} href={mt.url || undefined} target={mt.url ? "_blank" : undefined} rel={mt.url ? "noopener noreferrer" : undefined} style={{ padding: 16, background: `${C.accent}08`, borderRadius: 10, border: `1px solid ${C.accent}18`, display: "flex", gap: 14, alignItems: "center", textDecoration: "none" }}><div style={{ width: 44, height: 44, borderRadius: "50%", background: `${C.accent}12`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18, flexShrink: 0 }}>👤</div><div style={{ flex: 1 }}><div style={{ fontSize: 13, fontWeight: 600, color: C.text }}>{mt.name}</div><div style={{ fontSize: 12, color: C.textMuted }}>{mt.role}</div><div style={{ fontSize: 11, color: C.textDim }}>{mt.org}</div></div>{mt.url && <span style={{ color: C.textMuted }}>↗</span>}</W>; })}</div> : <div style={{ padding: 28, background: `${C.accent}08`, borderRadius: 12, border: `1px dashed ${C.accent}25`, textAlign: "center" }}><div style={{ fontSize: 13, color: C.textMuted }}>Coming soon</div></div>}</div></section>

      {/* ORGANIZATIONS */}
      <section ref={registerRef("organizations")} style={{ paddingTop: 32, borderTop: `1px solid ${C.border}` }}><div style={{ background: `linear-gradient(135deg, ${C.secondary}06, ${C.primary}04)`, borderRadius: 16, border: `1px solid ${C.border}`, padding: m ? "20px 16px" : "28px 24px" }}><div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 6 }}><span style={{ fontSize: 20 }}>🏛️</span><h2 style={{ fontSize: 20, fontWeight: 700, margin: 0, color: C.text }}>Organizations</h2></div><p style={{ fontSize: 13, color: C.textMuted, margin: "0 0 20px" }}>Affiliated, employed, and collaborated with</p>{ORG_GROUPS.map(g => { const orgs = ORGANIZATIONS.filter(o => o.group === g.key); if (!orgs.length) return null; return <div key={g.key} style={{ marginBottom: 20 }}><div style={{ fontSize: 12, fontWeight: 600, color: C.textMuted, marginBottom: 10 }}>{g.icon} {g.label}</div><div style={{ display: "grid", gridTemplateColumns: m ? "1fr" : "repeat(auto-fill, minmax(280px, 1fr))", gap: 8 }}>{orgs.map((o, i) => { const W = o.url ? "a" : "div"; return <W key={i} href={o.url || undefined} target={o.url ? "_blank" : undefined} rel={o.url ? "noopener noreferrer" : undefined} style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 14px", background: C.surface, borderRadius: 8, border: `1px solid ${C.border}`, textDecoration: "none" }}><div style={{ width: 32, height: 32, borderRadius: 7, background: `${C.secondary}10`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 16, flexShrink: 0 }}>{o.logo}</div><div style={{ flex: 1, minWidth: 0 }}><div style={{ fontSize: 13, fontWeight: 600, color: C.text, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{o.name}</div><div style={{ fontSize: 11, color: C.textMuted, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{o.role}</div></div>{o.period && <div style={{ fontSize: 10, color: C.textDim, flexShrink: 0 }}>{o.period}</div>}{o.url && <span style={{ color: C.textMuted, fontSize: 11 }}>↗</span>}</W>; })}</div></div>; })}</div></section>
...
const WorkPage = ({ initialProjectId }) => {
  ...
  return (
    <div style={{ paddingTop: 28, paddingBottom: 40 }}>
      <h1 style={{ fontSize: m ? 26 : 30, fontWeight: 300, margin: 0, color: C.text }}><span style={{ fontWeight: 700 }}>{S.pages.work.headline}</span></h1>
      {/* GitHub + Pubs zone */}
      <div style={{ display: "grid", gridTemplateColumns: m ? "1fr" : "1fr 1fr", gap: 12, marginTop: 24, padding: m ? 14 : 18, background: `linear-gradient(135deg, ${C.secondary}08, ${C.secondary}04)`, borderRadius: 14, border: `1px solid ${C.border}` }}>
        <a href={S.socials.find(s => s.name === "GitHub")?.url || "#"} target="_blank" rel="noopener noreferrer" style={{ display: "flex", alignItems: "center", gap: 14, padding: "16px 18px", background: C.surface, borderRadius: 10, border: `1px solid ${C.border}`, textDecoration: "none" }}><div style={{ width: 40, height: 40, borderRadius: 10, background: `${C.secondary}12`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18, fontWeight: 800, color: C.secondary, flexShrink: 0 }}>GH</div><div style={{ flex: 1 }}><div style={{ fontSize: 14, fontWeight: 600, color: C.text }}>GitHub</div><div style={{ fontSize: 11, color: C.textMuted }}>Repositories</div></div><span style={{ color: C.secondary }}>→</span></a>
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}><a href="https://www.researchgate.net/profile/Kim-Hwang-Yeo/research" target="_blank" rel="noopener noreferrer" style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 14px", background: C.surface, borderRadius: 10, border: `1px solid ${C.border}`, textDecoration: "none", flex: 1 }}><div style={{ width: 32, height: 32, borderRadius: 7, background: "#00d0a110", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, fontWeight: 800, color: "#00d0a1" }}>RG</div><div style={{ flex: 1, fontSize: 12, fontWeight: 600, color: C.text }}>ResearchGate</div><span style={{ fontSize: 12, color: "#00d0a1" }}>→</span></a><a href="https://orcid.org/0000-0001-8849-1534" target="_blank" rel="noopener noreferrer" style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 14px", background: C.surface, borderRadius: 10, border: `1px solid ${C.border}`, textDecoration: "none", flex: 1 }}><div style={{ width: 32, height: 32, borderRadius: 7, background: "#a6ce3910", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, fontWeight: 800, color: "#a6ce39" }}>ID</div><div style={{ flex: 1, fontSize: 12, fontWeight: 600, color: C.text }}>ORCID</div><span style={{ fontSize: 12, color: "#a6ce39" }}>→</span></a></div>
      </div>
      {/* Tabs */}
      <div style={{ display: "flex", gap: 0, marginTop: 28, borderBottom: `1px solid ${C.border}`, overflowX: "auto" }}>{[{ k: "projects", l: "Projects" }, { k: "publications", l: "Publications" }, { k: "writings", l: "Writings" }, { k: "links", l: "Links" }].map(t => <button key={t.k} onClick={() => setTab(t.k)} style={{ padding: "10px 18px", fontSize: 13, cursor: "pointer", background: "transparent", border: "none", color: tab === t.k ? C.text : C.textDim, borderBottom: tab === t.k ? `2px solid ${C.secondary}` : "2px solid transparent", fontWeight: tab === t.k ? 600 : 400, whiteSpace: "nowrap" }}>{t.l}</button>)}</div>
      {tab === "projects" && <div style={{ paddingTop: 20 }}><div style={{ display: "flex", gap: 6, marginBottom: 18, flexWrap: "wrap" }}>{[{ k: "all", l: "All" }, { k: "own", l: "My Projects" }, { k: "mentored", l: "Mentored" }].map(f => <button key={f.k} onClick={() => setProjectFilter(f.k)} style={{ padding: "5px 12px", fontSize: 12, borderRadius: 20, cursor: "pointer", background: projectFilter === f.k ? C.text : C.surface, color: projectFilter === f.k ? C.bg : C.textMuted, border: `1px solid ${projectFilter === f.k ? C.text : C.border}`, fontWeight: projectFilter === f.k ? 600 : 400 }}>{f.l}</button>)}<div style={{ width: 1, height: 22, background: C.border, alignSelf: "center" }} />{[{ k: "all", l: "All Themes" }, ...S.themes.map(t => ({ k: t.label, l: t.label }))].map(f => <button key={f.k} onClick={() => setThemeFilter(f.k)} style={{ padding: "5px 12px", fontSize: 12, borderRadius: 20, cursor: "pointer", background: themeFilter === f.k ? C.primary : C.surface, color: themeFilter === f.k ? "#fff" : C.textMuted, border: `1px solid ${themeFilter === f.k ? C.primary : C.border}`, fontWeight: themeFilter === f.k ? 600 : 400 }}>{f.l}</button>)}</div><div style={{ display: "grid", gridTemplateColumns: m ? "1fr" : "repeat(auto-fill, minmax(320px, 1fr))", gap: 14 }}>{filteredProjects.map(p => <ProjectCard key={p.id} project={p} onClick={setModalProject} />)}</div></div>}
      {tab === "publications" && <div style={{ paddingTop: 20 }}>{PUBLICATIONS.length > 0 ? <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>{PUBLICATIONS.map((p, i) => <div key={i} style={{ display: "flex", alignItems: "center", padding: "14px 18px", background: C.surface, borderRadius: 10, border: `1px solid ${C.border}`, flexWrap: m ? "wrap" : "nowrap", gap: 10 }}><div style={{ flex: 1, minWidth: 0 }}><div style={{ fontSize: 14, fontWeight: 600, color: C.text }}>{p.title}</div><div style={{ fontSize: 12, color: C.textMuted, marginTop: 2 }}>{p.venue} · {p.year}</div></div><Pill color={p.type === "paper" ? C.secondary : C.accent} accent small>{p.type}</Pill><a href={p.pdfUrl} style={{ padding: "5px 12px", background: C.surfaceHover, border: `1px solid ${C.border}`, borderRadius: 6, fontSize: 12, color: C.textMuted, textDecoration: "none" }}>↓ PDF</a></div>)}</div> : <div style={{ padding: 36, textAlign: "center", color: C.textDim, fontSize: 13 }}>No publications yet.</div>}</div>}
...
const InsightsPage = () => {
  ...
  return (
    <div style={{ paddingTop: 28, paddingBottom: 40 }}>
      <h1 style={{ fontSize: m ? 26 : 30, fontWeight: 300, margin: 0, color: C.text }}><span style={{ fontWeight: 700 }}>{P.headline}</span></h1>
      <p style={{ fontSize: 14, color: C.textMuted, marginTop: 8, maxWidth: 700, lineHeight: 1.7 }}>{P.bio}</p>
        ...
        <nav style={{ position: "sticky", top: 0, zIndex: 100, background: `${C.bg}e0`, backdropFilter: "blur(16px)", borderBottom: `1px solid ${C.border}` }}>
          <div style={{ maxWidth: 1200, margin: "0 auto", padding: "0 20px", height: 56, display: "flex", alignItems: "center" }}>
            ...
            <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 5 }}>
              ...
              <button onClick={() => setColorMode(cm => cm === "dark" ? "light" : "dark")} title={isLight ? "Switch to dark mode" : "Switch to light mode"} style={{
                padding: "3px 10px", borderRadius: 6, cursor: "pointer", fontSize: 11, fontWeight: 700, transition: "all 0.2s",
                background: isLight ? "#fef3c7" : C.surface,
                border: `1px solid ${isLight ? "#f59e0b66" : C.border}`,
                color: isLight ? "#92400e" : C.textMuted,
              }}>{colorMode === "light" ? "☀ Light" : "☾ Dark"}</button>
              {/* Accessibility toggle — high-contrast styled when NOT active so users who need it can find it */}
              <button onClick={() => setIsAccessible(a => !a)} title={isAccessible ? "Switch to default theme" : "Switch to high contrast"} style={{
                padding: "3px 10px", borderRadius: 6, cursor: "pointer", fontSize: 11, fontWeight: 700, transition: "all 0.2s",
