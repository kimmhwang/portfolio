      ...
      {highlights.map((h, i) => {
        ...
        return (
          <div key={i} style={{ padding: pad, borderRight: last ? "none" : "1px solid var(--line-soft)" }}>
            <div className="mono" style={{ fontSize: 11, letterSpacing: 1, textTransform: "uppercase", color: "var(--accent)", marginBottom: 9 }}>{h.tag}</div>
            ...
            <div style={{ marginTop: 14, display: "flex", flexDirection: "column", gap: 12 }}>
              {c.mentors.map((p, i) => (
                <div key={i} style={{ display: "flex", alignItems: "center", gap: 12 }}>
                  <span style={{ flexShrink: 0, width: 34, height: 34, borderRadius: "50%", background: "var(--img-bg)", boxShadow: "inset 0 0 0 1px var(--img-edge)", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "var(--mono)", fontSize: 11, letterSpacing: 0.5, color: "var(--accent)" }}>{mentorInitials(p.name)}</span>
                  <div>
    ...
    <div className="wrap stack" style={{ display: "grid", gridTemplateColumns: "1.1fr 1fr", gap: 56 }}>
      ...
      <div>
        ...
        {c.insights.map((ins, i) => (
          <a key={i} className="pd-link" href={ins.href} target="_blank" rel="noopener noreferrer" style={{ display: "block", padding: "16px 0", borderTop: "1px solid var(--line)" }}>
...
export default function PortfolioV3() {
  ...
  return (
    <div className="pv2" data-theme={theme}>
      ...
      {SITE_META.banner && <div className="wip-banner">{SITE_META.banner}</div>}
